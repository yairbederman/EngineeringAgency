...
6. **Layer & Dependency Analysis**:
   ...
   - **Cross-Repository Check** (Multi-Workspace):
     - **Read `${SYSTEM_ARCH_OUTPUT}/analysis/service-topology.json`** to understand the full service chain
     - Use `callsServices` and `calledBy` to trace the data path
     - If bug involves API calls, check `${SYSTEM_ARCH_OUTPUT}/analysis/cross-service-apis.json` for the exact endpoint contract
